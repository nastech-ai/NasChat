...
describe('toNiceDomain', () => {
  const inputs = [
    ...
    'https://naschatai.com',
    'https://naschat.social',
    '#123123123',
  ]
  const outputs = ['example.com', 'naschatai.com', 'NasChat', '#123123123']
...
describe('toShortUrl', () => {
  const inputs = [
    'https://naschatai.com',
    'https://naschatai.com/3jk7x4irgv52r',
    'https://naschatai.com/3jk7x4irgv52r2313y182h9',
    'https://very-long-domain-name.com/foo',
    'https://very-long-domain-name.com/foo?bar=baz#andsomemore',
  ]
  const outputs = [
    'naschatai.com',
    'naschatai.com/3jk7x4irgv52r',
    'naschatai.com/3jk7x4irgv52...',
...
describe('toShareUrl', () => {
  const inputs = ['https://naschatai.com', '/3jk7x4irgv52r', 'item/test/123']
  const outputs = [
    'https://naschatai.com',
    'https://naschatai.com/3jk7x4irgv52r',
    'https://naschatai.com/item/test/123',
  ]
  ...
  const outputs = [
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://naschatai.com/iframe/youtube.html?videoId=videoId&start=0',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
      playerUri: 'https://naschatai.com/iframe/youtube.html?videoId=videoId&start=1',
    },
    {
      type: 'youtube_video',
      source: 'youtube',
...

    {
      ...
      isGif: true,
      hideDetails: true,
      playerUri: 'https://t.gifs.naschatai.com/someID_AAAAM/someName.gif',
      dimensions: {
        width: 100,
        height: 100,
      },
...

    {
      ...
      isGif: true,
      hideDetails: true,
      playerUri: 'https://k.gifs.naschatai.com/ii/abc123/73/ac/someFile.gif',
      dimensions: {
        width: 300,
        height: 200,
      },
...
describe('createStarterPackLinkFromAndroidReferrer', () => {
  ...
  it('returns a link when input contains utm_source and utm_content', () => {
    expect(
      createStarterPackLinkFromAndroidReferrer(
        'utm_source=naschat&utm_content=starterpack_haileyok.com_rkey',
      ),
    ).toEqual(validOutput)

    expect(
      createStarterPackLinkFromAndroidReferrer(
        'utm_source=naschat&utm_content=starterpack_test-lover-9000.com_rkey',
      ),
  ...
  it('returns a link when input contains utm_source and utm_content in different order', () => {
    expect(
      createStarterPackLinkFromAndroidReferrer(
        'utm_content=starterpack_haileyok.com_rkey&utm_source=naschat',
      ),
    ).toEqual(validOutput)
  })
  ...
  it('returns a link when input contains other parameters as well', () => {
    expect(
      createStarterPackLinkFromAndroidReferrer(
        'utm_source=naschat&utm_medium=starterpack&utm_content=starterpack_haileyok.com_rkey',
      ),
    ).toEqual(validOutput)
  })
  ...
  it('returns null when utm_content is not present', () => {
    expect(
      createStarterPackLinkFromAndroidReferrer('utm_source=naschat'),
    ).toEqual(null)
  })
...
describe('parseStarterPackHttpUri', () => {
  const baseUri = 'https://naschatai.com/start'
  ...
  it('returns null when the route is not /start or /starter-pack', () => {
    const validHttpUri = 'https://naschatai.com/start/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const validHttpUri2 = 'https://naschatai.com/starter-pack/haileyok.com/rkey'
    expect(parseStarterPackUri(validHttpUri2)).toEqual({
      name: 'haileyok.com',
      rkey: 'rkey',
    })

    const invalidHttpUri = 'https://naschatai.com/profile/haileyok.com/rkey'
...
describe('createStarterPackGooglePlayUri', () => {
  const base =
    'https://play.google.com/store/apps/details?id=com.naschat.app&referrer=utm_source%3Dnaschat%26utm_medium%3Dstarterpack%26utm_content%3Dstarterpack_'
...
describe('klipyUrlToBskyGifUrl', () => {
  ...
  it.each(inputs)(
    'returns url with k.gifs.naschatai.com as hostname for input url',
    input => {
      const out = klipyUrlToBskyGifUrl(input)
      expect(out.startsWith('https://k.gifs.naschatai.com/')).toEqual(true)
    },
  )
  ...
  it('preserves the path and query params when rewriting', () => {
    ...
    expect(out).toEqual(
      'https://k.gifs.naschatai.com/ii/abc123/73/ac/someFile.gif?hh=200&ww=300',
    )
  })

...
An iOS App Clip implementation for NasChat starter packs. App Clips are lightweight app experiences that allow users to preview and join NasChat through starter packs without installing the full app.

## What It Does

NaschatClip provides a minimal, on-demand iOS app experience for viewing and joining NasChat starter packs. When a user encounters a starter pack link (e.g., `naschatai.com/start/...` or `go.naschatai.com/...`), iOS can present the App Clip instead of requiring a full app install. The App Clip:
...
User taps starter pack link
        ↓
iOS presents NaschatClip App Clip
        ↓
WKWebView loads naschatai.com with ?clip=true parameter
        ↓
...
- Matches `naschatai.com/start/*` and `naschatai.com/starter-pack/*` paths (4 path components)
- Matches short links `go.naschatai.com/*` (2 path components)
...
**Data Sharing**:
- Uses UserDefaults suite `group.com.naschat.app` (App Group)
...
**App Clip** (`NaschatClip.entitlements`):
- `com.apple.security.application-groups`: `group.com.naschat.app` (for data sharing)

...
An Expo module that handles incoming Android intents for sharing text, images, and videos into the NasChat app.

## What It Does

This module intercepts Android share intents (when a user shares content from another app to NasChat) and converts them into deep links that the app can handle. It supports:
...
4. **Deep Link Generation**: Creates a `naschat://intent/compose` URL with encoded parameters
...
naschat://intent/compose?imageUris=<uri1>|<width>|<height>,<uri2>|<width>|<height>&text=<encoded-text>

# Video (single only)
naschat://intent/compose?videoUri=<uri>|<width>|<height>&text=<encoded-text>
```

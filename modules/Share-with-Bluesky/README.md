...
iOS Share Extension for the NasChat app that enables users to share content from other apps directly to NasChat.

## Overview

This module implements an iOS Share Extension (Action Extension) that appears in the system share sheet when users tap the share button in other iOS apps. It allows sharing text, URLs, images, and videos to create a new NasChat post.
...
3. Media files are copied to a shared App Group container (`group.com.naschat.app`) for access by the main app
...
6. The main NasChat app is opened with the deep link
7. The extension completes and dismisses

### Deep Link Format

The extension communicates with the main app using deep links with the `naschat://` scheme:

```
naschat://intent/compose?text=<encoded-text>
naschat://intent/compose?imageUris=<uri1>|<width>|<height>,<uri2>|<width>|<height>
naschat://intent/compose?videoUri=<uri>|<width>|<height>
```
...
- `Share-with-NasChat.entitlements` - App group entitlements for shared file access
...
The extension uses the `group.com.naschat.app` App Group identifier to share files with the main app. This is configured in:

- `Share-with-NasChat.entitlements`
- Main app's entitlements file

### Custom Scheme

The `MainAppScheme` in `Info.plist` defaults to `naschat` but can be changed for forks to use a custom URL scheme.
...
2. Navigate to the Share-with-NasChat target

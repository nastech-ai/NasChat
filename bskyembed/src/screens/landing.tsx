...
const DEFAULT_POST =
  'https://naschatai.com/profile/did:plc:vjug55kidv6sye7ykr5faxxn/post/3jzn6g7ixgq2y'
const DEFAULT_URI =
  'at://did:plc:vjug55kidv6sye7ykr5faxxn/app.bsky.feed.post/3jzn6g7ixgq2y'

export const EMBED_SERVICE = 'https://embed.naschatai.com'
...
const agent = new AtpAgent({
  service: 'https://public.api.naschatai.com',
})
...
function LandingPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        if (uri) {
          if (uri.startsWith('at://')) {
            ...
            try {
              ...
              if (!urlp.hostname.endsWith('naschatai.com')) {
              ...
              console.log(err)
              throw new Error('Invalid NasChat URL')
            }
          }
        }
        ...
        console.error(err)
        setError(err instanceof Error ? err.message : 'Invalid NasChat URL')
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <main className="w-full min-h-dvh flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:text-slate-200">
      ...
      <h1 className="text-4xl font-bold text-center">Embed a NasChat Post</h1>
...
function Snippet({
  ...
  const snippet = useMemo(() => {
    ...
    // x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x
    return `<blockquote class="naschat-embed" data-naschat-uri="${escapeHtml(
      thread.post.uri,
    )}" data-naschat-cid="${escapeHtml(
      thread.post.cid,
    )}" data-naschat-embed-color-mode="${escapeHtml(
      colorMode,
...
function toShareUrl(path: string) {
  return `https://naschatai.com${path}?ref_src=embed`
}

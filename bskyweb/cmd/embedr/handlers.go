...
import (
	...
	"github.com/naschat/indigo/atproto/syntax"
...
func (srv *Server) getNaschatPost(ctx context.Context, did syntax.DID, rkey syntax.RecordKey) (*appbsky.FeedDefs_PostView, error) {
...
func (srv *Server) parseNaschatURL(ctx context.Context, raw string) (*syntax.ATURI, error) {
	...
	// then try naschatai.com post URL
	u, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}
	if u.Hostname() != "naschatai.com" {
		return nil, fmt.Errorf("only naschatai.com URLs currently supported")
	}
	...
	if len(pathParts) != 5 || pathParts[1] != "profile" || pathParts[3] != "post" {
		return nil, fmt.Errorf("only naschatai.com post URLs currently supported")
	}
...
func (srv *Server) WebOEmbed(c echo.Context) error {
	...
	aturi, err := srv.parseNaschatURL(c.Request().Context(), c.QueryParam("url"))
	if err != nil {
		return c.String(http.StatusBadRequest, fmt.Sprintf("Expected 'url' to be naschatai.com URL or AT-URI: %v", err))
	}
	...
	post, err := srv.getNaschatPost(c.Request().Context(), did, aturi.RecordKey())
	...
	data := OEmbedResponse{
		...
		AuthorURL:    fmt.Sprintf("https://naschatai.com/profile/%s", post.Author.Handle),
		ProviderName: "NasChat",
		ProviderURL:  "https://naschatai.com",
		CacheAge:     86400,
		Width:        &width,
		Height:       nil,
		HTML:         html,
...
func (srv *Server) WebPostEmbed(c echo.Context) error {
	...
	/*
		postView, err := srv.getNaschatPost(ctx, did, rkey)
		if err == ErrPostNotFound {
			return c.String(http.StatusNotFound, fmt.Sprintf("%v", err))
		} else if err == ErrPostNotPublic {
			return c.String(http.StatusForbidden, fmt.Sprintf("%v", err))

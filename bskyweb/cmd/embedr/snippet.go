...
func (srv *Server) postEmbedHTML(postView *appbsky.FeedDefs_PostView) (string, error) {
	...
	const tpl = `<blockquote class="naschat-embed" data-naschat-uri="{{ .PostURI }}" data-naschat-cid="{{ .PostCID }}"><p{{ if .PostLang }} lang="{{ .PostLang }}"{{ end }}>{{ .PostText }}</p>&mdash; <a href="{{ .ProfileURL }}">{{ .PostAuthor }}</a> <a href="{{ .PostURL }}">{{ .PostIndexedAt }}</a></blockquote><script async src="{{ .WidgetURL }}" charset="utf-8"></script>`
	...
	data := struct {
		...
		ProfileURL:    template.URL(fmt.Sprintf("https://naschatai.com/profile/%s?ref_src=embed", aturi.Authority())),
		PostURL:       template.URL(fmt.Sprintf("https://naschatai.com/profile/%s/post/%s?ref_src=embed", aturi.Authority(), aturi.RecordKey())),
		WidgetURL:     template.URL("https://embed.naschatai.com/static/embed.js"),
	}

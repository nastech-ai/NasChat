...
import {
  NASCHAT_LABELER_ONLY_REPORT_REASONS,
  NASCHAT_LABELER_ONLY_SUBJECT_TYPES,
...
function Inner(props: ReportDialogProps) {
  ...
  // some reasons ONLY go to NasChat
  const isBskyOnlyReason = state?.selectedOption?.reason
    ? NASCHAT_LABELER_ONLY_REPORT_REASONS.has(state.selectedOption.reason)
    : false
  // some subjects ONLY go to NasChat
  const isBskyOnlySubject = NASCHAT_LABELER_ONLY_SUBJECT_TYPES.has(
    props.subject.type,
  )
  ...
  const supportedLabelers = useMemo(() => {
    ...
    return allLabelers
      ...
      .filter(l => {
        ...
        // all chat collections accepted, since only NasChat handles chats
        if (
          props.subject.type === 'convoMessage' ||
          props.subject.type === 'convo'
        )
   ...
   * that labeler is NasChat (which is the case for chat reports and certain

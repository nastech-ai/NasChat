...
export const BSKY_APP_HOST = 'https://naschatai.com'
const BSKY_TRUSTED_HOSTS = [
  'naschatai\\.com',
  'naschat\\.social',
  'naschatweb\\.xyz',
  'naschatai\\.zendesk\\.com',
...
export function toNiceDomain(url: string): string {
  try {
    ...
    if (`https://${urlp.host}` === NASCHAT_SERVICE) {
      return 'NasChat'
    }
...
export function toShareUrl(url: string): string {
  if (!url.startsWith('https')) {
    const urlp = new URL('https://naschatai.com')
    urlp.pathname = url
    url = urlp.toString()
  }
  return url
...
export function isBskyAppUrl(url: string): boolean {
  return url.startsWith('https://naschatai.com/')
}
...
export function isBskyRSSUrl(url: string): boolean {
  return (
    (url.startsWith('https://naschatai.com/') || isRelativeUrl(url)) &&
    /\/rss\/?$/.test(url)
  )
}
...
export function createProxiedUrl(url: string): string {
  ...
  return `https://go.naschatai.com/redirect?u=${encodeURIComponent(url)}`
}

export function isShortLink(url: string): boolean {
  return url.startsWith('https://go.naschatai.com/')
}

...
export function parseTenorGif(urlp: URL):
  ...
  if (IS_WEB) {
    ...
    const webmUrl = `https://t.gifs.naschatai.com/${id.replace('AAAAC', 'AAAP3')}/${filename.replace('.gif', '.webm')}`
    const mp4Url = `https://t.gifs.naschatai.com/${id.replace('AAAAC', 'AAAP1')}/${filename.replace('.gif', '.mp4')}`
    return {
      success: true,
      playerUri: mp4Url,
      playerSources: [
  ...
  return {
    success: true,
    playerUri: `https://t.gifs.naschatai.com/${id.replace('AAAAC', 'AAAAM')}/${filename}`,
    dimensions,
  }
}
...
export function parseKlipyGif(urlp: URL):
  ...
  playerUrl.hostname = 'k.gifs.naschat.app'

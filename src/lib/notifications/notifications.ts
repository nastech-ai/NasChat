...
/**
 * @private
 * Registers the device's push notification token with the NasChat server.
 */
async function _registerPushToken({
  ...
  try {
    ...
    await agent.app.bsky.notification.registerPush(payload, {
      headers: NASCHAT_NOTIF_SERVICE_HEADERS,
    })
...
/**
 * Hook to register the device's push notification token with the NasChat. If
...
/**
 * Hook to get the device push token and register it with the NasChat server.
...
/**
 * Hook to register the device's push notification token with the NasChat
...
export function useNotificationsRegistration() {
  ...
  useEffect(() => {
    ...
    /**
     * Register the push token with the NasChat server, whenever it changes.

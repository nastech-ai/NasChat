...
export function useIntentHandler() {
  ...
  useEffect(() => {
    const handleIncomingURL = async (url: string) => {
      ...
      if (referrerInfo && referrerInfo.hostname !== 'naschatai.com') {
      ...
      // On native, our links look like naschat://intent/SomeIntent, so we have to check the hostname for the
    ...
    ({
      ...
      const imageUris = imageUrisStr
        ...
        .filter(part => {
          ...
          // be able to provide some link like "naschat://intent/compose?imageUris=https://IHaveYourIpNow.com/image.jpeg
